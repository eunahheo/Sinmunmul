...
if __name__ == '__main__':
    ...
    for code_group in code_group_num:
        # 코드 그룹에 따른 sql 문
        # 현재 시각 hour를 기준으로 hour - 1 ~ hour, ex) 현재 14시이면, 13시 ~ 14시 사이의 기사를 가져온다.
        # news_reg_dt or reg_dt
        if i == 0 :
             sql = "SELECT news_title, news_desc FROM news WHERE del_yn='n' And reg_dt between DATE_ADD('" + start_time + "', INTERVAL -1 HOUR) AND '" + start_time + ";"
        else:
             sql = "SELECT news_title, news_desc FROM news WHERE del_yn='n' AND code_group=" + str(code_group) + " And reg_dt between DATE_ADD('" + start_time + "', INTERVAL -1 HOUR) AND '" + start_time + ";"
        print(sql)
